         ...
         return new AbstractIterator<BlockPos>() {
            ...
            protected BlockPos computeNext() {
               if (-this.currRY > yCutOffMin) {
                  ...
                  BlockPos pos = this.mutablePos.m_122178_(c1 + this.currRX, c2 + this.currRY, c3 + this.phase * this.offset);
                  ++this.offset;
                  return pos;
               } else {
                  if (this.phase == 1) {
         ...
         return new AbstractIterator<BlockPos>() {
            ...
            protected BlockPos computeNext() {
               if (-this.currRY > yCutOffMin) {
                  ...
                  if (distance > minr2 && !((float)distance <= (float)maxr2 * Mth.m_14036_(rand.m_188501_(), rDifference, 1.0F))) {
                     if (distance <= maxr2) {
                        ++this.offset;
                        return this.computeNext();
                     } else {
         ...
         return new AbstractIterator<BlockPos>() {
            ...
            protected BlockPos computeNext() {
               if (-this.currR > radius) {
                  ...
                  BlockPos pos = this.mutablePos.m_122178_(c1 + this.currR, c2, c3 + this.phase * this.offset);
                  ++this.offset;
                  return pos;
               } else {
                  if (this.phase == 1) {
