...
public class PathResult<T extends Callable<Path>> {
   protected PathFindingStatus status;
   private static boolean threadException = false;
   private volatile boolean pathReachesDestination;
   private Path path;
   private Future<Path> pathCalculation;
   private T job;
   private boolean pathingDoneAndProcessed;

   public PathResult() {
      this.status = PathFindingStatus.IN_PROGRESS_COMPUTING;
      this.pathReachesDestination = false;
      this.path = null;
      this.pathCalculation = null;
      this.job = null;
      this.pathingDoneAndProcessed = false;
   }
