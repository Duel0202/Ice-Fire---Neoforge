...
public class LayerBipedArmor<T extends LivingEntity & IAnimatedEntity, M extends ModelBipedBase<T>, A extends ModelBipedBase<T>> extends RenderLayer<T, M> {
   ...
   private void renderEquipment(PoseStack matrixStackIn, MultiBufferSource bufferIn, T entityIn, EquipmentSlot slotType, int packedLightIn, A modelIn) {
      ...
      if (itemstack.m_41720_() instanceof ArmorItem) {
         ...
         if (armoritem.m_40402_() == slotType) {
            ...
            this.renderArmorItem(matrixStackIn, bufferIn, packedLightIn, flag1, modelIn, 1.0F, 1.0F, 1.0F, this.getArmorResource(entityIn, itemstack, slotType, (String)null));
         }
      }
   ...
   private void renderArmorItem(PoseStack matrixStackIn, MultiBufferSource bufferIn, int packedLightIn, boolean p_241738_5_, A modelIn, float red, float green, float blue, ResourceLocation armorResource) {
      VertexConsumer ivertexbuilder = ItemRenderer.m_115184_(bufferIn, RenderType.m_110431_(armorResource), false, p_241738_5_);
