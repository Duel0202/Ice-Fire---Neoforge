...
public class GorgonTempleStructure extends IafStructure {
   public static final Codec<GorgonTempleStructure> CODEC = RecordCodecBuilder.mapCodec((instance) -> {
   ...
   public GorgonTempleStructure(StructureSettings config, Holder<StructureTemplatePool> startPool, Optional<ResourceLocation> startJigsawName, int size, HeightProvider startHeight, Optional<Types> projectStartToHeightmap, int maxDistanceFromCenter) {
   ...
   public StructureType<?> m_213658_() {
      return (StructureType)IafStructureTypes.GORGON_TEMPLE.get();
   }
   ...
   public static GorgonTempleStructure buildStructureConfig(BootstapContext<Structure> context) {
      ...
      Holder<StructureTemplatePool> graveyardHolder = templatePoolHolderGetter.m_255043_(IafStructurePieces.GORGON_TEMPLE_START);
      return new GorgonTempleStructure(new StructureSettings(context.m_255420_(Registries.f_256952_).m_254956_(IafBiomeTagGenerator.HAS_GORGON_TEMPLE), new HashMap(), Decoration.SURFACE_STRUCTURES, TerrainAdjustment.BEARD_THIN), graveyardHolder, Optional.empty(), 2, ConstantHeight.f_161945_, Optional.of(Types.WORLD_SURFACE_WG), 16);
   }
}
